...
import {QuizScience} from "../../../services/index"
...
function Science() {
  ...
  const Science = new QuizScience();
  const [index,setIndex] = useState(Science.currentIndex)
const [queItem,setqueItem] =useState(Science.questions[index])
const [showresults , setShowResults] = useState(Science.showres)
const [score,setScore] = useState(Science.scoreCount);
const [showcolor,setShowColor] = useState(false)
const changeque = ()=>{
  Science.nextQuestions();
  if(Science.questions.length - 1 === index){
    setShowResults(true)
  }
  else{
    setIndex(index + 1)
...
console.log(queItem)
useEffect(()=>{
setqueItem(Science.questions[index])
},[index])
  ...
  return (
...

   {
    ...
    {showcolor ?<div className='flex items-center justify-center w-[200px] p-2'>{color === "green" ? <p className='bg-[green] text-white w-[200px] p-2 flex items-center justify-center'>Correct Answer</p>:<p className='bg-[red] text-white w-[300px] p-2'>Wrong Answer</p>}</div>:null}
